""" 
adding additional functionalities for 
centralized trianing and decentralized execution 
"""
import numpy as np 
...
#####################################################################################
### runner for centralized trianing and decentralized execution 
#####################################################################################
...
class CTDERunner(BaseRunner):
    ...
    - overrides `pack_transition` method to support adding more stuff to buffer
    """
    def __init__(self, scheme, env, mac, logger,
...
#####################################################################################
### run one step each time 
#####################################################################################

class CTDEStepRunner(CTDERunner):
    """ wrap upon vectorized env to collect episdoes (vec env collect steps)
    """
    ...
    def run(self, render=False):
        ...
        torch_actions, step_info = self.mac.step(torch_obs, 
            explore=self.is_training, infos=self.infos)
        ...
        transition_data = self.pack_transition(
            self.obs, torch_actions, next_obs, rewards, dones, 
...
class CTDEEpisodeRunner(CTDERunner):
    """ wrap upon vectorized env to collect episdoes (vec env collect steps)
        for centralized training and decentralized execution
    """
    def __init__(self, scheme, env, mac, logger, batch_size, max_episode_len, 
    ...
    def run(self, render=False):
        ...
        for et_i in range(self.max_episode_len):
            ...
            # [dict (B,A)]*N -> [ [(A,)]*N ]*B or [ [tuple (A,)]*N ]*B
            torch_actions, step_info = self.mac.step(torch_obs, 
                explore=self.is_training, infos=infos)
            actions = self.group_actions(torch_actions)
